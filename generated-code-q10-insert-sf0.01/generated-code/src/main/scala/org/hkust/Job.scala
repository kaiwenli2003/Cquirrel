...
object Job {
   ...
   def main(args: Array[String]) {
      ...
      val inputpath = "file:///root/autodl-tmp/Cquirrel-release/DemoTools/DataGenerator/input_data_insert_sf0.01.csv"
      val outputpath = "file:///root/autodl-tmp/Cquirrel-release/experiment_script/output_q10_insert_sf0.01.csv"
   ...
   private def getStream(env: StreamExecutionEnvironment, dataPath: String): DataStream[Payload] = {
      ...
      header match {
         ...
         action = "Insert"
         relation = "lineitem"
         val i = Tuple6(cells(5).toDouble,cells(8),cells(15),cells(0).toLong,cells(6).toDouble,cells(3).toInt)
         cnt = cnt + 1
         ctx.output(lineitemTag, Payload(relation, action, cells(0).toLong.asInstanceOf[Any],
         Array[Any](i._1,i._2,i._3,i._4,i._5,i._6),
         Array[String]("L_EXTENDEDPRICE","L_RETURNFLAG","L_COMMENT","ORDERKEY","L_DISCOUNT","LINENUMBER"), cnt))
         case "-LI" =>
         action = "Delete"
         relation = "lineitem"
         val i = Tuple6(cells(5).toDouble,cells(8),cells(15),cells(0).toLong,cells(6).toDouble,cells(3).toInt)
